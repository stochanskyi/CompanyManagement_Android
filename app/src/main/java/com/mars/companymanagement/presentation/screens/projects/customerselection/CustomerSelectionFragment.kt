...
@AndroidEntryPoint
class CustomerSelectionFragment : Fragment(R.layout.fragment_customer_selection) {

    companion object {
        const val SELECTED_USER_DATA_KEY = "selected_user_data_key"
    }
    ...
    private fun initObservers(binding: FragmentCustomerSelectionBinding) {
        viewModel.customersLoadingLiveData.observe(viewLifecycleOwner) {
        ...
        viewModel.customerSelectedLiveData.observe(viewLifecycleOwner) {
            ...
            navController.previousBackStackEntry?.savedStateHandle?.set(SELECTED_USER_DATA_KEY, it)
